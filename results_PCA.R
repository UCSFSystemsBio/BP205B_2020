...
km_results <- fread('/wynton/home/students/snanda/rds/bp205/analysis/survival/results/survival_results.csv') %>%
  dplyr::filter(is.finite(KM_HZ_0.95_upper),str_detect(signature,'FC_1')) %>% 
  dplyr::filter(str_detect(signature,'MDA'))

km_results_data <- as.matrix(km_results[,-1])
rownames(km_results_data) <- km_results$signature
